...
export class DevtoolsLoggingService implements OnInit, OnDestroy {
  ...
  private detectDevTools(): void {
    ...
    devtools.toString = () => {
      if (!this.devToolsOpened) {
        ...
        this.clearConsoleAndWarn();
        this.stopDetection(); // Dừng kiểm tra sau khi phát hiện
      } else {
        // Trường hợp DevTools đã đóng
        if (this.devToolsOpened) {
    ...
    console.log('%c', devtools); // Kích hoạt toString
  }
